...
<div v-if="searchTerm.query != ''" class="bg-white my-2 rounded-lg shadow-lg mx-auto max-w-md">
    <div v-for="place in searchTerm.response" :key="place.id">
        <button class="px-3 my-2 hover: text-indigo-600 hover:font-bold w-full text-left" @click="searchForecast(place.id)">{{place.name + ', ' + place.region +', ' + place.country}}</button>
    </div>
</div>
</template>
